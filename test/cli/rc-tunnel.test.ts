...
describe('CLI: rc-tunnel command', () => {
  it('module exports isDevtunnelAvailable function', async () => {
    const mod = await import('@jerrettdavis/squad-cli/commands/rc-tunnel');
  ...
  it('isDevtunnelAvailable returns a boolean', async () => {
    const { isDevtunnelAvailable } = await import('@jerrettdavis/squad-cli/commands/rc-tunnel');
    const result = isDevtunnelAvailable();
    expect(typeof result).toBe('boolean');
  });

  it('getMachineId returns the system hostname', async () => {
    const { getMachineId } = await import('@jerrettdavis/squad-cli/commands/rc-tunnel');
    const id = getMachineId();
    expect(id).toBe(os.hostname());
  });

  it('getGitInfo returns repo and branch from cwd', async () => {
    const { getGitInfo } = await import('@jerrettdavis/squad-cli/commands/rc-tunnel');
  ...
  it('destroyTunnel does not throw when no tunnel active', async () => {
    const { destroyTunnel } = await import('@jerrettdavis/squad-cli/commands/rc-tunnel');
